...
export default function App() { const [active, setActive] = useState<Tab>("Domain Intel"); const panel = { "Domain Intel": <DomainIntelPanel />, "Network Recon": <NetworkReconPanel />, "Social Profiling": <SocialProfilingPanel />, "Breach Check": <BreachCheckPanel />, "Report Engine": <ReportBuilder /> }[active]; return <main><header><p className="eyebrow">Academic investigation workspace</p><h1>OSINT Toolkit</h1><p>Run focused public-source checks or build a consolidated investigation report.</p></header>{apiConfigurationError && <div className="error-banner"><strong>Setup required.</strong> {apiConfigurationError}</div>}<nav aria-label="Toolkit features">{tabs.map(tab => <button key={tab} className={active === tab ? "tab active" : "tab"} onClick={() => setActive(tab)}>{tab}</button>)}</nav>{panel}</main>; }
